...
from gramatica_regular import (
    parse_gramatica_regular,
    gr_para_afn,
    af_para_gramatica,
    gramatica_para_texto,
)
import json
...
def _gerar_gramaticas(afn, afd):
    gr_afn = gramatica_para_texto(af_para_gramatica(afn)) if afn is not None else None
    gr_afd = gramatica_para_texto(af_para_gramatica(afd)) if afd is not None else None
...
def minimizar():
    try:
        ...
        return _render_visualizar(afd_min_imagem=afd_min.desenhar("afd_min"))
    except Exception as e:
...
def gramatica():
    try:
        ...
        gramatica_parsed = parse_gramatica_regular(texto)
        afn = gr_para_afn(gramatica_parsed)
        afd = afn.converter_para_afd()
        _salvar_session(afn, afd)
        return _render_visualizar(
...
def af_para_gramatica_view():
    try:
        ...
        return _render_visualizar(gramatica_afn=gr_afn, gramatica_afd=gr_afd)
    except Exception as e:
